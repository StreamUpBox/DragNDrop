...
import { Business,Branch, MainModelService,UserBusiness, Tables,Types,User, BusinessCategory,NotificationService } from '@enexus/flipper-components';
...
export class CreateUpdateBusinessComponent   implements OnInit {
  ...
  user:User=null;
  constructor(private router:Router,protected _notificationSvc: NotificationService,private formBuilder: FormBuilder,private model:MainModelService) {
   }
  ...
  onSubmit() {
      ...
      if (this.registerForm.invalid) {
        this._notificationSvc.error('Create Business', 'We need you to complete all of the required fields before we can continue');
          return;
      }

     
    
      let formBusinessData:Business={
        name:this.registerForm.value.name,
        categoryId:parseInt(this.registerForm.value.businessCategoryId),
        currency:this.registerForm.value.currency,
        country:this.registerForm.value.country,
        businessUrl:this.registerForm.value.name+'.flipper.rw',
        typeId:parseInt(this.registerForm.value.typeId),
        active:true,
        createdAt:new Date(),
        updatedAt:new Date()
     }
     ...
     let createdBusiness:any=this.model.create<Business>(Tables.business,formBusinessData);

     let formBranchData:Branch={
       name:this.registerForm.value.branchName,
       active:true,
       businessId:createdBusiness?parseInt(createdBusiness['id']):0
     ...
     let formUserBusinesData:UserBusiness={
    ...
    if(this.hasBusiness){
     ...
     setTimeout(()=>{
      ...
      this._notificationSvc.success('Create Business', 'Business '+formBusinessData.name+' created successfully!');
