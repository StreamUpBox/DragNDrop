...
import { OrderEvent, STATUS, ORDERTYPE, Order, VariantEvent, Variant,ShoppingEvent, Shoppings, CalculateTotalClassPipe } from '@enexus/flipper-components';
...
set gottenVariant(value: Variant[]) {
...
set currentOrder(value: Order) {
  ...
  public searchPosVariant(event) {
    if (event) {
       this.gottenVariant=this.filterByValue(this.variants, event);
    }
